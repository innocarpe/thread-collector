...
├── scripts/discover_threads.py            # 후보 유저 발굴 (코퍼스 멘션 마이닝)
...
Key routing rules:
- "/discover-threads", "discover", "트렌드 유저 찾아줘", "핫한 유저 추천해줘", "새로 누구 수집할지 골라줘", "멘션된 유저 정리" → invoke `discover-threads` skill. 절대 바로 `/collect` 으로 넘어가지 말 것 — 사용자가 리포트 보고 직접 고르는 흐름이 이 스킬의 핵심.
...
변경 시 `types/post.ts` · `scripts/classify.py` · `scripts/collect.py` 같이 수정.
...
2. `.thread-collector-config` 파일 생성:
   ...
   ```
3. repo 루트에서 설정 확인: `python3 scripts/fetch_threads.py --check-config`

## 실행 규칙

- 스크립트는 **repo 루트에서 상대경로**로 실행한다.
