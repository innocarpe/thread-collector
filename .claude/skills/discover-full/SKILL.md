...
- **키워드** (`--query`) — "키워드:", "검색어:", 따옴표 안 텍스트, 또는 문맥상 핵심 단어. 여러 개 가능. **없으면 관심사 기반 자동 도출**.
- **해시태그** (`--hashtag`) — `#` 포함/미포함 모두 OK. 여러 개 가능. **없으면 관심사 기반 자동 도출**.
- **결과 수** — "N명", "N개" 언급 시 `--limit N` (기본 25)
- **enrich 수** — 언급 없으면 기본 15 유지

### 자동 키워드/해시태그 도출 (신규)

사용자가 `--query` 나 `--hashtag` 를 명시하지 않은 경우, 스크립트는 **관심사 슬러그에서 자동 생성**한다:

- 관심사 해석 우선순위: `--interest` CLI > `.thread-collector-interests.json` > 코퍼스 상위 3개 카테고리 자동 도출
- 각 관심 카테고리에 매핑된 사전 정의 keyword/hashtag 목록에서 **최대 3개** 뽑아 사용
  - 예: `ai-llm` → queries `["AI 수익화", "바이브코딩", "LLM"]`, hashtags `["ai", "vibecoding", "llm"]`
  - 예: `monetization` → queries `["부수입", "온라인 수익화", "디지털노마드"]`, hashtags `["부업", "수익화", "sidehustle"]`
- stderr 에 `[auto] --query 미지정 → 관심사 기반 자동 생성: [...]` 로깅

즉 **사용자가 `/discover-full` 만 외치면 시스템이 알아서 관심사 → 키워드/해시태그 → 풀 파이프라인 실행**한다.
...
# (1) 아무 인자 없이 풀 파이프라인 — 관심사 기반 자동 키워드/해시태그 도출
...
# (4) 관심사 명시 + 자동 키워드 도출 (특정 주제 집중 탐색)
python3 scripts/discover_threads.py \
  --sources corpus,search,hashtag \
  --interest monetization,startup-philosophy \
  --enrich \
  --enrich-limit 15 \
  --limit 25
```
