...
|---------|------|---------------------|----------|
| programmingzombie | Android 대량 포트폴리오 + AdMob 운영 | **가장 직접 복제 가능한 모델** — 다만 2026-04-29 Play/web 역추적 기준 기존 Dave's·Rich Kim 외 `DeveloperKhy`, `developerdoga` 라벨까지 확장되어 계정보다 **앱→fingerprint 역추적**이 더 중요해짐 | 50~60개 → 하루 $100 / 150개 → 월급 이상 / 공개 Play 최고 버킷 100K+ (`com.dave.keywordhelper`, `com.dave.spellchecker`) |
...
| dalgom_bami | iOS 구독앱 + Paid UA (직장인 대상) | iOS 구독 모델 실전 가이드 — Sensor Tower 조사법, Meta Ad Library, 10배 차별화, RevenueCat | 직접 iOS 구독 앱 수익화 + 소프트웨어 마에스트로 멘토링 6년 |
...
> **2026-04-28 업데이트**: dalgom_bami 아티클("마케팅 싫어하는 개발자를 위한 iOS 구독앱 수익화 가이드") 반영. iOS 구독 + Paid UA 트랙을 1순위로 격상. 기존 AdMob 대량 포트폴리오 모델은 병행 트랙으로 유지.

**1순위 — dalgom_bami + sebastian-rohl 복합 모델 (iOS 구독 + Paid UA)**
- iOS 단일 플랫폼 집중: ARPU가 Android 대비 3~5배, Face ID 결제 마찰 최소
- 소형 팀이 만든 iOS 구독 앱 틈새 찾기 → 10배 가치 차별화 → RevenueCat 구독 결제
- Meta Ads + Apple Search Ads Paid UA 중심 (월 30~50만원 예산으로 시간 부족 메움)
- 마케팅을 회피하는 것이 아니라 Paid로 외주화 — 콘텐츠 인플루언서 역량 불필요
- Apple Small Business Program 즉시 신청: 수수료 30% → 15% (소급 적용 안 됨)
- 선행 리서치: Sensor Tower 매출 20~100위 → TikTok 검색 + Meta Ad Library로 경쟁 광고 분석

**2순위 — programmingzombie + hussein-el-feky 복합 모델 (AdMob 대량 포트폴리오)**
- Android/iOS 양쪽 개발 가능한 TemperStone의 강점 활용
- 소형 유틸리티 앱 대량 출시 → AdMob 수익화 → House ads 교차 홍보
- 첫 앱부터 하루 $1 기준으로 빠르게 판단, 50~60개 시점에 하루 $100 목표
- 단, dalgom_bami 경고: 마케팅 역량 없이 Android 대량 배포는 분산 위험 → 1순위 트랙이 자리 잡은 뒤 병행
...
## Part 2.3 — iOS 구독 모델 심화 전략 (dalgom_bami)

> 소스: dalgom_bami X 아티클 "아직 퇴사하지 마세요. 마케팅 싫어하는 개발자를 위한 가장 편한 구독앱 수익화 가이드" + Threads/@dalgom.bami (113개 게시글). 직장인 iOS 개발자 + 소프트웨어 마에스트로 멘토링 6년 경험 기반.

### 2.3.1 왜 iOS 구독이 AdMob 대량 모델보다 먼저인가

dalgom_bami의 핵심 명제: 마케팅을 싫어하거나 인플루언서 역량이 없는 개발자에게 AdMob 광고수익 모델은 함정이다. 광고 모델은 "압도적 트래픽 = 인플루언서 역량"이 필수인데, 직장 다니면서 인플루언서까지 하는 건 비현실적이다. 반면 구독 모델은 Paid UA(메타, Apple Search Ads)로 시간을 돈으로 사서 대체할 수 있다.

**iOS vs Android 구독 ARPU**: iOS가 Android 대비 3~5배. iPhone 사용자는 기기 셋업 시 결제 수단을 미리 등록 → Face ID 한 번으로 구독 완료. Android는 결제 단계 이탈률이 훨씬 높다.

**양 플랫폼 동시 운영 함정**: 빌드 시간 3배, QA 인프라 2배, 광고 소재 2배, 페이월 전략 분산. 직장인 1인이 감당 불가. iOS 한 사이클을 끝까지 돌린 뒤 확장 여부 판단.

### 2.3.2 아이템 선정 — Sensor Tower 20~100위 조사법

1. **카테고리 좁히기**: 도메인 이해도 있는 카테고리 1~2개 (Health & Fitness, Productivity, Lifestyle 등). 광고 소재에서 그 카테고리 사용자의 언어를 써야 하기 때문.
2. **매출 순위 20~100위 스캔**: 1~20위는 대기업 앱(Paid 10억 써서 매출 5억 구조). 20~100위에서 "작은 팀이 만든 것 같고 Paid도 많이 안 하는데 매출 나는 앱" 찾기.
3. **소형 팀 시그널**: 개발자 이름이 개인명에 가까움 / 앱 수 1~3개 / 인디 개발자 향 디자인 / 소셜 없거나 리뷰 답변이 개발자 직접 톤.
4. **목표 구간**: 월 추정 매출 1천만~5천만원 사이 소형 팀 앱. "이 시장은 소형 팀도 들어갈 수 있다"는 검증이 끝난 것.

### 2.3.3 경쟁 앱 분석 — TikTok + Meta Ad Library (사실상 답지)

**TikTok 검색**: 앱 이름 검색 → 사용자 획득 패턴, 광고 톤, 인플루언서 언급 파악. 광고 소재 아이디어가 막힐 때 반복 참조.

**Meta Ad Library**: facebook.com/ads/library에서 앱 이름 검색.
- 광고 갯수: 동시 집행 소재 수
- 캠페인 기간: 기간 길수록 큰 금액 집행 중
- 소재 패턴: Hook → Demo → CTA 구조 분석
- 타깃 국가: 어느 나라에 집중하는지

이 두 개만 제대로 분석해도 광고 소재 전략의 70%가 풀린다.

### 2.3.4 10배 가치 차별화 원칙

카피 금지. 원조 앱은 사용자 데이터, ASO 스코어, 리뷰가 이미 쌓여 있어 똑같이 만들면 절대 못 이긴다. 대신 "그 앱 사용자 중 일부에게 10배 가치를 줄 방향"으로 타깃을 좁힌다.

| 원조 앱 | 차별화 방향 |
|--------|-----------|
| "모두를 위한 명상 앱" | "수험생 전용 5분 명상 앱" |
| "기능 빵빵한 가계부" | "한 줄 기록 미니멀 가계부" |
| "AI로 사진 보정" | "30대 여성 인물 사진 특화 AI 보정" |

큰 기업은 좁은 타깃에 깊이 못 들어간다. 그게 1인 개발자의 무기.

### 2.3.5 Paid UA 중심 마케팅 (월 30~50만원)

콘텐츠 마케팅, SEO, 커뮤니티 빌딩은 직장인에게 비현실적. Paid UA로 시간 부족을 돈으로 메운다.

- **Meta Ads**: 광고 소재 A/B 테스트 중심. Hook→Demo→CTA 구조 준수.
- **Apple Search Ads**: sebastian-rohl 실험치 — 75€ 집행, 35 installs, 설치당 약 2.14€, 탭→설치 전환 60%.
- 초기 소재 방향: TikTok + Meta Ad Library에서 경쟁 앱 소재를 먼저 분석한 뒤 차별화 포인트 명확히 하고 제작.

### 2.3.6 인프라 셋업 체크리스트

- **RevenueCat** 연결: StoreKit 2 구독 결제를 며칠 내 붙일 수 있음. MRR, 활성 구독자, 28일 매출 추적.
- **Apple Small Business Program 즉시 신청**: developer.apple.com/app-store/small-business-program. 매출 0원이어도 신청 가능. 신청한 다음 달부터 수수료 15% 적용 (소급 없음). 30% → 15% = 개발자 수취분 21% 증가.
- **App Store Connect Agreements, Tax, Banking**: 출시 전 필수 완료. 은행 계좌 SWIFT 코드·영문 주소 정확히 입력 안 하면 정산 거부.
- **Apple Developer Program 개인 계정($99/년)**: 개인 계정으로 시작. 법인 전환은 매출 발생 후 재검토.

### 2.3.7 아이템 선정 최종 워크플로 (dalgom_bami 7단계)

```
1. Sensor Tower — 카테고리 1~2개 선정
        ↓
2. 매출 순위 20~100위 훑기
        ↓
3. "소형 팀 + 의미있는 매출" 시그널 앱 5~10개 추리기
        ↓
4. 각 앱 TikTok 검색 → 사용자 획득 패턴 파악
        ↓
5. 각 앱 Meta Ad Library 검색 → 광고 운영 여부 + 소재 패턴 확인
        ↓
6. 그 앱들의 사용자 페르소나에서 더 좁히거나 더 깊이 갈 수 있는 niche 찾기
        ↓
7. 그 niche에 10배 가치를 줄 수 있는 컨셉 하나로 좁히기
```

이 7단계를 최소 1주일 이상 진행. 며칠 만에 결정하면 이후 6개월이 날아간다.
...
**iOS 구독 트랙 (1순위)**
- [ ] **트랙 선택 확정**: iOS 구독 + Paid UA 트랙으로 시작. AdMob 대량 모델은 이후 병행.
- [ ] **Sensor Tower 조사**: 관심 카테고리 1~2개 → 매출 20~100위 훑기 → 소형 팀 앱 5~10개 추리기 (dalgom_bami 7단계 1~3단계)
- [ ] **경쟁 앱 분석**: TikTok 검색 + Meta Ad Library — 소재 패턴, 타깃 국가, Hook 구조 파악 (dalgom_bami 7단계 4~5단계)
- [ ] **첫 앱 컨셉 확정**: 10배 가치 차별화 기준 적용, 타깃 좁히기. ASO 인기 점수 20 초과 + 난이도 70 미만 + 경쟁 앱 월매출 $5,000 초과 확인 (roman-koch 필터 병용)
- [ ] **Apple Developer Program 가입 + Apple Small Business Program 즉시 신청** (매출 0원이어도 신청. 소급 없음)
- [ ] **App Store Connect Agreements, Tax, Banking 완료** (출시 전 필수)
- [ ] **RevenueCat 계정 준비 + StoreKit 2 구독 설계**
- [ ] **익명 빌드 인 퍼블릭 채널 1개 세팅**: X 또는 Threads (스텔스 ~2026-06-30)

**병행 준비 (AdMob 트랙)**
- [ ] **AdMob 계정 + Google Play Console 준비 확인**
- [ ] **순수익 대시보드 초안 생성**: AdMob 수익 + Google Ads 비용 같은 시트
- 필요 도구: Sensor Tower, Meta Ad Library, TikTok, 앱브레인, 구글 트렌드, RevenueCat, App Store Connect, AdMob, Cursor
...
### 5.12 처음부터 iOS + Android 동시 운영

> "직장인 1인 개발자가 양 플랫폼을 운영하기 시작하면 빌드 시간이 세 배, QA 인프라가 두 배, 광고 소재가 두 배가 된다. 둘 다 어중간하게 된다."
> — dalgom_bami 아티클

**대응**: iOS 단일 플랫폼으로 한 사이클(출시→광고→페이월 최적화→ASO→손익분기)을 끝까지 돌린다. 안드로이드 확장은 그 뒤에 판단. iOS만 해서 후회한 사례는 없고, 처음부터 안드로이드도 해서 후회한 사례는 많다.

### 5.13 구독 모델인데 Paid UA 없이 유기 트래픽만 기다리기

> "콘텐츠 마케팅, SEO, 커뮤니티 빌딩은 시간이 많은 풀타임 인디해커에게 어울리는 전략이다. 직장 다니면서 그것까지 다 하는 건 비현실적이다."
> — dalgom_bami 아티클

**대응**: iOS 구독 모델의 핵심 채널은 Meta Ads + Apple Search Ads Paid UA다. 월 30~50만원 예산을 마케팅에 쓸 여력이 있어야 이 트랙이 유효하다. 예산이 없으면 AdMob 대량 포트폴리오(2순위) 트랙을 먼저 가동해 현금흐름을 만든다.
